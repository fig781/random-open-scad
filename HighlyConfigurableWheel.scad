...
wheelWidth     = 12;     // The width (or thickness) of the wheel at the rim.
tireCSDiameter = 4;     // Cross-sectional diameter (CS) -- How thick is the tire rubber?
tireID         = 20;    // Internal diameter (ID) -- How wide is the inside opening? 
...
timingHolePad  = [0,0,0]; // The [inside,middle,outside] padding for the timing holes
...
staggerOffset     = 0;            // A distance to offset the staggered rows. 
...
spaceBetweenTires = 0;            // For o-rings/v-grooves, the space between each tire, if more than one 
...
spokeWidth        = 10;          // This is how wide each spoke is.
...
outerHoleDiameter = 15;         // For "circlefit", the diameter of the outer holes, or zero for none
...
hubZOffset           = -7;     // The Z position of the hub, negative numbers from the surface of the wheel 
...
shaftDiameter        = 5.3;          // The diameter of the motor shaft
shaftFlatDiameter    = 4.8;          // The diameter of the motor shaft at the flat, or shaftDiameter for no flat.

setScrewCount        = 1;          // The number of set screws/nuts to render, spaced evenly around the shaft 
setScrewDiameter     = 3.5;          // The diameter of the set screw. 3 is the default for an M3 screw. 
...
setScrewNutOffset    = -1;          // The distance to offset the nut from the center of the material. -/+ = in/out
			...
			difference() {
				union() {
					assign( d = wheelDiameter - (rimHeight*2) ) {
						...
						} else if ( spokeStyle == "spork" ) {
							sporkSpokes( d, wheelWidth, numberOfSpokes,
								spokeWidth, spiralSpoke);
						}
					}
...
//Spork pattern spokes
module sporkSpokes( wheelDiameter, wheelWidth, numberofSpokes ) {
	echo( "Diamonds Style..." ); 
    for (step = [0:numberofSpokes-1]) {
        rotate( a=step*(360/numberofSpokes), v=[0, 0, 1])
        sporkSpoke( wheelDiameter, wheelWidth );
    }
}
bowlWidth = 30; 
bowlLength = 45; 
...
module spoon() {
	difference() {
		...
		union() {	
			rotate([0,bowlAngle,0]) 
			translate([-(bowlLength)/2,0,bowlCutOffset]) 
			difference() {
		
				bowlShape(bowlLength, bowlWidth, bowlDepth);
				// hollow
				bowlShape(bowlLength-bowlThickness, bowlWidth-bowlThickness, bowlDepth-bowlThickness);
				// cut in half 
				translate([0,0,(bowlDepth+1+bowlCutOffset)/2 - bowlCutOffset]) 
					cube([bowlLength+1, bowlWidth+1, bowlDepth+1+bowlCutOffset], center=true); 

				// fork
				for(i=[-1,0,1]) {
					translate([-bowlLength/2,5*i,-bowlDepth])
						linear_extrude(height=bowlDepth*2) 
							scale([.9,0.2]) 
								circle(r=10);
				}
			}
			
			translate([handleLength/2-10,0,-handleHeight/2]) 
				cube([handleLength,handleWidth,handleHeight], center=true);
		}
...
module bowlShape(len, wid, dep) {
	scale([len/wid, 1, dep/wid*2]) 
		sphere(r=wid/2);
}
